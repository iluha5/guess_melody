...
const redrawCircle = (circle, radius, animation) => {
  ...
  circle.setAttributeNS(null, `r`, radius); // radius.greet()
...
window.initializeCountdown = (step) => {
  const element = document.querySelector(`.timer-line`);
  const radius = parseInt(element.getAttributeNS(null, `r`), 10);
  const timer = document.querySelector(`.timer-value`);
  const currStep = step ? step : 0;
  return window.animation.animate(window.animation.getAnimation(currStep, 1000, 120), (animation) => {
